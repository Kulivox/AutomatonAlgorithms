...
namespace AutomatonAlgorithms.AutomatonProcedures.CompareAutomatons
{
    public class AutomatonComparer : IAutomatonProcedure
    {
        ...
        {
            Logger.Warn(
                "This procedure is intended for comparison of minimised and canonised automatons. Other inputs might cause the output to be innacurate");
            if (automata.Count != 2 || strings.Count != 0)
                throw new ProcedureException(
                    "AutomatonComparer: bad amount of inputs, this procedure requires two automaton variables to compare");
        ...
        private void CompareAutomatonsAndSaveResult(Automaton left, Automaton right)
        {
            ...
            foreach (var ds in differingStates) sb.Append(ds);

            sb.Append("Different transitions:\n");
            foreach (var dt in differingTransitions) sb.Append(dt);
